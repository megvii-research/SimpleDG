...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('epochs', 120, lambda r: int(r.uniform(0, 120)))
    # TODO: nonlinear classifiers disabled
    _hparam('nonlinear_classifier', False,
            lambda r: bool(r.choice([False, False])))
    _hparam('lr_scheduler', 'CosineAnnealingLR',
            lambda r: r.choice(['CosineAnnealingLR', 'Constant', 'WarmupCosineAnnealingLR']))
    _hparam('optimizer', 'SGD',
            lambda r: r.choice(['SGD', 'Adam', 'AdamW']))
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('ms_p', 0.5, lambda r: 0.5)
        _hparam('ms_alpha', 0.1, lambda r: 0.1)
        _hparam('ms_eps', 1e-6, lambda r: 1e-6)
        _hparam('ms_type', "random", lambda r: lambda r: r.choice(["random", "crossdomain"]))
        _hparam('ms_layers', [1,], lambda r: r.choice([[1,], [1,2], [1,2,3]]))
