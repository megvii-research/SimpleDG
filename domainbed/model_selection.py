...
class SelectionMethod:
    ...
    def hparams_accs(self, records):
        ...
        return (records.group('args.hparams_seed')
            ...
            .sorted(key=lambda x: x[0]['source_out'])[::-1]
        )

    @classmethod
    def every_acc_dict(self, records):
        """
        Given all records from a single (dataset, algorithm, test env) pair,
...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, record):
        ...
        for i in itertools.count():
            ...
            if i not in test_envs:
                source_in_acc_keys.append(f'env{i}_in_acc')
                source_out_acc_keys.append(f'env{i}_out_acc')
            else:
                target_in_acc_keys.append(f'env{i}_in_acc')
                target_out_acc_keys.append(f'env{i}_out_acc')

            every_keys.append(f'env{i}_out_acc')
            every_keys.append(f'env{i}_in_acc')
        source_out = np.mean([record.get(key, 0.0) for key in source_out_acc_keys])
        source_in = np.mean([record.get(key, 0.0) for key in source_in_acc_keys])
        target_in = np.mean([record.get(key, 0.0) for key in target_in_acc_keys])
        target_out = np.mean([record.get(key, 0.0) for key in target_out_acc_keys])
        if source_out == 0.0:
            source_out = source_in
        return {
            **{key: record.get(key, 0.0) for key in every_keys},
            'target_in': target_in,
    ...
    def run_acc(self, run_records):
        ...
        if not len(test_records):
            return None
        return test_records.map(self._step_acc).argmax('source_out')

class LastEpochIIDAccuracySelectionMethod(SelectionMethod):
    """Picks argmax(mean(env_out_acc for env in train_envs))"""
    name = "training-domain validation set"
    ...
    def _step_acc(self, record):
        ...
        return {
            'val_acc': np.mean([record.get(key, 0.0) for key in val_env_keys]),
    ...
    def run_acc(self, run_records):
        ...
        if not len(test_records):
            return None
        chosen_records = test_records.sorted(lambda r: r['step'])[-1:]
        return Q(chosen_records).map(self._step_acc).argmax('val_acc')
