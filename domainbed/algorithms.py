...
from domainbed.optimizer import SAM, GSAM, ESAM
from domainbed.augment import FMix, FMixup
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.feature_size = self.featurizer.n_outputs
        if hparams.get('mlp'):
            mlp_dim = hparams['mlp']
            self.featurizer.network.fc = nn.Sequential(
                nn.Linear(self.featurizer.n_outputs, mlp_dim),
                nn.ReLU(),
                nn.Linear(mlp_dim, self.featurizer.n_outputs)
            )

        # use fmixup by default
        if hparams.get('fmixup', True):
            self.mixer = FMixup(
                mixup_alpha=10, cutmix_alpha=10, fmix_alpha=10, 
                label_smoothing=0.1, size=input_shape[1:], num_classes=num_classes)
        elif hparams.get('mixup'):
            self.mixer = mixup.Mixup(mixup_alpha=0.1, cutmix_alpha=1, label_smoothing=0, num_classes=num_classes)
        elif hparams.get('fmix'):
            self.mixer = FMix(alpha=1, size=input_shape[1:], num_classes=num_classes)
        else:
            self.mixer = None

        if torch.cuda.device_count() > 1:
            print("Let's use", torch.cuda.device_count(), "GPUs!")
            self.featurizer = nn.DataParallel(self.featurizer, device_ids=range(torch.cuda.device_count()))
        ...
        if self.hparams['optimizer'] == 'Adam':
            ...
            self.optimizer = torch.optim.SGD(self.classifier.parameters(), lr=self.hparams["lr"],
                momentum=0.9,
                weight_decay=self.hparams['weight_decay'])

        T_max =  self.hparams["steps_per_epoch"] * self.hparams["epochs"]
        if self.hparams['lr_scheduler'] == 'WarmupCosineAnnealingLR':
            warmup_iters = self.hparams["steps_per_epoch"] * 5 #
            lr_max = 1.0
            lr_min = lr_max*1e-3
            lambda0 = lambda cur_iter: cur_iter / warmup_iters if  cur_iter < warmup_iters else \
                (lr_min + 0.5*(lr_max-lr_min)*(1.0+math.cos( (cur_iter-warmup_iters)/(T_max-warmup_iters)*math.pi)))
            self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=[lambda0])
        elif self.hparams['lr_scheduler'] == 'Constant':
            lambda0 = lambda cur_iter: 1.0
            self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=[lambda0])
        else:
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.mixer is not None:
            all_x, all_y = self.mixer(all_x, all_y)
    ...
    #     featurizer_state_dict = {k.replace('module.', ''): v for k,v in state_dict.items() if 'fc' not in k}
    #     self.featurizer.network.load_state_dict(featurizer_state_dict)
    #     classifier_state_dict = {k.replace('module.fc.', ''): v for k,v in state_dict.items() if 'fc' in k}
    #     self.classifier.load_state_dict(classifier_state_dict)

class MultiHeadDropout(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.classifiers = torch.nn.ModuleList([networks.Classifier(
            ...
            num_classes,
            self.hparams['nonlinear_classifier'],
            dropout_rate=0.1) for _ in range(hparams.get('num_heads', 4))])

        for cls in self.classifiers:
            self.optimizer.add_param_group({'params': cls.parameters()})
    ...
    def predict(self, x):
        ...
        preds = []
        for cls in self.classifiers:
            preds.append(cls(feature))
        return torch.stack(preds).mean(dim=0)


class CrossNorm(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        hparams['cross_norm'] = True
        hparams['nonlinear_classifier'] = True
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.cross_norm = networks.CrossNorm()
    ...
    def crossed_ce_loss(self, pred, target, n):
        target = torch.repeat_interleave(target.unsqueeze(1), n, dim=1)
        target = target.reshape(n*n, -1).squeeze(1)
        loss = F.cross_entropy(pred, target)
        return loss

    def style_loss(self, feat_mean, feat_std, domain_mask):
        n, c, _ = feat_mean.shape
        mean_loss = ((feat_mean.reshape(1, n, c) - feat_mean.reshape(n, 1, c))**2).sum(axis=2)
        std_loss = ((feat_std.reshape(1, n, c) - feat_std.reshape(n, 1, c))**2).sum(axis=2)
        style_loss = (mean_loss * domain_mask + std_loss * domain_mask).sum() / domain_mask.sum()
        return style_loss
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if False:
            cross_feat, feat_mean, feat_std = self.cross_norm(feat)
            cross_pred = self.classifier(cross_feat)

            domain_num = len(minibatches)
            minibatch_size = minibatches[0][0].shape[0]
            domain_label = torch.linspace(0, domain_num - 1, domain_num).unsqueeze(1).to(self.device)
            domain_label = domain_label.expand(domain_num, minibatch_size)
            domain_label = domain_label.reshape(1, -1)
            domain_mask = domain_label == domain_label.reshape(-1, 1)
            # domain_mask = ~domain_mask
            cross_loss = self.crossed_ce_loss(cross_pred, all_y, all_x.shape[0])

            style_loss = self.style_loss(feat_mean, feat_std, domain_mask)

            loss = ce_loss + cross_loss + 0.01 * style_loss
            res = {'loss': loss.item(), 'loss_ce': ce_loss.item(), 'loss_x': cross_loss.item(), 'loss_style': style_loss.item()}
        else:
            loss = ce_loss
            res = {'loss': loss.item()}
...
class SCL(ERM):
    def update(self, minibatches, unlabeled=None, eps=1e-9):
        features = [self.featurizer(x) for x, _ in minibatches]
        targets = [y.unsqueeze(1) for _, y in minibatches]

        all_feature = torch.cat(features, dim=0)
        all_target = torch.cat(targets, dim=0)
        ce_loss = F.cross_entropy(self.classifier(all_feature), all_target.squeeze(1))

        features = [f / (f.norm(p=2, dim=1, keepdim=True) + eps) for f in features]
        distances = [f @ f.t() for f in features]
        neg_masks = [y != y.t() for y in targets]
        neg_sim = [(d * m).sum(dim=1) / (m.sum(dim=1) + eps) for d, m in zip(distances, neg_masks)]

        all_feature = torch.cat(features, dim=0)
        all_dist = all_feature @ all_feature.t()
        pos_mask = all_target == all_target.t()
        pos_sim = (all_dist * pos_mask).sum(dim=1) / (pos_mask.sum(dim=1) + eps)

        contra_loss = sum([s.mean() for s in neg_sim]) / len(neg_sim) - pos_sim.mean() + 2
        ...
        return {'loss': loss.item(), 'loss_ce': ce_loss.item(), 'loss_contra': contra_loss.item()}
...
class ERM_SAM(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer = SAM(
            self.network.parameters(), base_optimizer, rho=self.hparams['rho'],
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        grad_norm = self.optimizer.step(closure)
        self.optimizer.zero_grad()

        return {'loss': loss.item(), 'grad_norm': grad_norm.item()}


class ERM_GSAM(ERM_SAM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        base_optimizer = torch.optim.SGD
        self.optimizer = GSAM(
            self.network.parameters(), base_optimizer,
            rho=self.hparams['rho'], alpha=self.hparams['alpha'],
            lr=self.hparams['lr'], momentum=0.9,
            weight_decay=self.hparams['weight_decay'])
...
class ERM_ESAM(ERM_SAM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        base_optimizer = torch.optim.SGD
        self.optimizer = ESAM(
            self.network.parameters(), base_optimizer,
            rho=self.hparams['rho'], beta=self.hparams['beta'], gamma=self.hparams['gamma'],
            lr=self.hparams['lr'], momentum=0.9,
            weight_decay=self.hparams['weight_decay'])
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss, grad_norm = self.optimizer.step(closure)
        self.optimizer.zero_grad()
...
class CLIP_ViT(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CLIP_ViT, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.ViT(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
            num_classes,
            self.hparams['nonlinear_classifier'])
...
class MixStyle(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        hparams["do_mixstyle"] = True
        super(MixStyle, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
...
class SelfRegCos(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SelfRegCos, self).__init__(input_shape, num_classes, num_domains,
                                   hparams)
        self.num_classes = num_classes
        def get_loss(out_1, out_2):
            out_1 = F.normalize(out_1, dim=-1)
            out_2 = F.normalize(out_2, dim=-1)
            pos_sim = torch.sum(out_1 * out_2, dim=-1)
            loss = 1- pos_sim.mean()
            return loss
        self.MSEloss = get_loss
        input_feat_size = self.featurizer.n_outputs
        hidden_size = input_feat_size if input_feat_size==2048 else input_feat_size*2
...
class SelfRegCosF(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SelfRegCosF, self).__init__(input_shape, num_classes, num_domains,
                                   hparams)
        self.num_classes = num_classes
        def get_loss(out_1, out_2):
            out_1 = F.normalize(out_1, dim=-1)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # regularization
        L_ind_logit = 0.0 * self.MSEloss(output, output_2)
        L_hdl_logit = 0.0 * self.MSEloss(output, output_3)
        L_ind_feat = 1.0 * self.MSEloss(feat, feat_2)
        L_hdl_feat = 1.0 * self.MSEloss(feat, feat_3)
...
class MovingAvg(nn.Module):
    def __init__(self, network, rm_optimizer=False, update_bn=False):
        super().__init__()
        if isinstance(network, MovingAvg):
        ...
        self.sma_start_iter = 2
        self.global_iter = 0
        self.sma_count = 0
        self.start_step = -1
        self.end_step = -1

    def update(self, network, start_step=None, end_step=None):
        if isinstance(network, MovingAvg):
            network = network.network

        self.step()
        self.update_param(network)
        if self.update_bn:
            self.update_bn_statistic(network)

        if start_step is not None:
            if self.sma_count == 1:
                self.start_step = start_step
        else:
            self.start_step = self.global_iter

        if end_step is not None:
            self.end_step = end_step
        else:
            self.end_step = self.global_iter

        return {k: v.item() for k, v in self.get_update_norm(network).items()}
    ...
    @torch.no_grad()
    def update_param(self, network):
        if self.global_iter>=self.sma_start_iter:
            for (name_q, param_q), (name_k, param_k) in zip(network.named_parameters(), self.network.named_parameters()):
    ...
    def get_named_buffer(self, network):
        buffer_dict = {}
        for name, buffer in network.named_buffers():
            n, sub_name = name.rsplit('.', 1)
            if sub_name in ['running_mean', 'running_var', 'num_batches_tracked']:
                buffer_dict.setdefault(n, {})[sub_name] = buffer

        return buffer_dict

    def update_bn_statistic(self, network, eps=1e-9):
        buffer_dict = self.get_named_buffer(network)
        sma_buffer_dict = self.get_named_buffer(self.network)
        if self.global_iter>=self.sma_start_iter:
            for name in buffer_dict:
                buffer = buffer_dict[name]
                sma_buffer = sma_buffer_dict[name]
                sma_buffer['running_mean'].data = (self.sma_count * sma_buffer['running_mean'] * buffer['running_var'] + sma_buffer['running_var'] * buffer['running_mean']) /\
                    (self.sma_count * buffer['running_var'] + sma_buffer['running_var'] + eps)
                sma_buffer['running_var'].data = (self.sma_count + 1) * sma_buffer['running_var'] * buffer['running_var'] /\
                    (self.sma_count * buffer['running_var'] + sma_buffer['running_var'] + eps)
        else:
            for name in buffer_dict:
                sma_buffer_dict[name]['running_mean'].data = buffer_dict[name]['running_mean'].data.clone()
                sma_buffer_dict[name]['running_var'].data = buffer_dict[name]['running_var'].data.clone()
                sma_buffer_dict[name]['num_batches_tracked'].data = buffer_dict[name]['num_batches_tracked'].data.clone()

    def get_update_norm(self, network):
        updates = {'running_mean': 0, 'running_var': 0, 'param': 0}

        buffer_dict = self.get_named_buffer(network)
        sma_buffer_dict = self.get_named_buffer(self.network)
        for name in buffer_dict:
            updates['running_mean'] += ((buffer_dict[name]['running_mean'] - sma_buffer_dict[name]['running_mean'])**2).sum()
            updates['running_var'] += ((buffer_dict[name]['running_var'] - sma_buffer_dict[name]['running_var'])**2).sum()

        for param_q, param_k in zip(network.parameters(), self.network.parameters()):
            updates['param'] += ((param_q - param_k)**2).sum()
...
class SWAD(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.update_bn = hparams.get('update_bn', False)
        self.segment_swa = MovingAvg(self.network, update_bn=self.update_bn)

        self.n_converge = hparams.get('n_converge', 3)
        self.n_tolerance = hparams.get('n_tolerance', 6)
        self.tolerance_ratio = hparams.get('tolerance_ratio', 0.3)

        self.converge_Q = deque(maxlen=self.n_converge)
        self.smooth_Q = deque(maxlen=self.n_tolerance)

        self.final_model = MovingAvg(self.network, update_bn=self.update_bn)
    ...
    def state_dict(self):
        return self.final_model.state_dict()

    def load_state_dict(self, state_dict, strict: bool = True):
        state_dict = {k[len('network.'):]: v for k, v in state_dict.items() if k.startswith('network.')}
        self.segment_swa.network.load_state_dict(state_dict, strict)
        self.final_model.network.load_state_dict(state_dict, strict)
        return self.network.load_state_dict(state_dict, strict)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        sma_update = self.segment_swa.update(self.network)
        sma_loss = F.cross_entropy(self.segment_swa.predict(all_x), all_y)
        sma_update.update({'loss': loss.item(), 'loss_sma': sma_loss.item()})

        return sma_update

    def predict(self, x):
        return self.segment_swa.predict(x)
    ...
    def update_and_evaluate(self, val_loss):
        if self.dead_valley:
            return

        frozen = copy.deepcopy(self.segment_swa.cpu())
        ...
        if not self.is_converged:
            ...
            if min_idx == 0:
                ...
                self.final_model = MovingAvg(untilmin_segment_swa, update_bn=self.update_bn)
    ...
    def get_final_model(self):
        ...
        if not self.dead_valley:
            ...
            while self.smooth_Q:
                ...
                self.final_model.update(segment_swa, end_step=segment_swa.end_step)
...
class SWA(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.update_bn = hparams.get('update_bn', True)
        self.sma = MovingAvg(self.network, update_bn=self.update_bn)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        sma_update = self.sma.update(self.network)
        sma_loss = F.cross_entropy(self.predict(all_x), all_y)
        sma_update.update({'loss': loss.item(), 'loss_sma': sma_loss.item()})

        return sma_update

    def reset(self):
        print('reset sma network')
        self.sma = MovingAvg(self.network, update_bn=self.update_bn)

    def state_dict(self):
        return self.sma.state_dict()

    def load_state_dict(self, state_dict, strict: bool = True, load_sma: bool = True):
        state_dict = {k[len('network.'):]: v for k, v in state_dict.items() if k.startswith('network.')}
        if torch.cuda.device_count() > 1:
            state_dict = {k.replace('network', 'module.network') if 'module' not in k else k: v for k, v in state_dict.items()}
        else:
            state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}

        if load_sma:
            self.sma.network.load_state_dict(state_dict, strict)
    ...
    def predict(self, x):
        return self.sma.predict(x)


class SWA_Distill(SWA):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)

        self.T = hparams.get('temperature', 2)
        self.ce_loss_w = hparams.get('ce_loss_w', 0.3)
        self.kl_loss_w = hparams.get('kl_loss_w', 0.7)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        logits_stu = self.network(all_x)
        ce_loss = F.cross_entropy(logits_stu, all_y)

        logits_tea = self.sma.predict(all_x).detach()
        sma_loss = F.cross_entropy(logits_tea, all_y)
        kd_loss = self.kl_loss(logits_stu, logits_tea, self.T)

        loss = self.ce_loss_w * ce_loss + self.kl_loss_w * kd_loss
        ...
        sma_update = self.sma.update(self.network)
        sma_update.update({
            'total_loss': loss.item(), 'ce_loss': ce_loss.item(),
            'kd_loss': kd_loss.item(), 'sma_loss': sma_loss.item()
        })

        return sma_update

    def kl_loss(self, logits_student, logits_teacher, temperature=2):
        log_pred_stu = F.log_softmax(logits_student / temperature, dim=1)
        pred_tea = F.softmax(logits_teacher / temperature, dim=1)
        loss_kl = F.kl_div(log_pred_stu, pred_tea, reduction='sum') * self.T**2 / logits_student.shape[0]
        return loss_kl
...
if __name__ == '__main__':
    ...
    hparams = hparams_registry.default_hparams('CORALP', 'PACS')
    hparams['resnet18'] = True
    hparams['mixup'] = True
    hparams['mmd_gamma'] = 0.1
    hparams['steps'] = 50000
    hparams['pretrained'] = False
    hparams['steps_per_epoch'] = 1000
    hparams['rho'] = 0.05
    hparams['beta'] = 0.5
    hparams['gamma'] = 0.5
    hparams['lr'] = 1e-3
    hparams['backbone'] = 'resnet50'
    hparams['cross_norm'] = False

    if torch.cuda.is_available():
        device = 'cuda'
    else:
        device = 'cpu'
    algo = MultiHeadDropout((3, 224, 224), 7, 4, hparams=hparams)
    for name, parameters in algo.named_parameters():
        print(name, ':', parameters.size())

    print('state dict')
    for k in algo.state_dict():
        print(k)
    print('load state dict')
    algo.load_state_dict(algo.state_dict())

    algo.to(device)
    algo.train()

    def debug_info():
        for n, p in algo.network.named_buffers():
            print('network buffer', n, p)
            break
        for n, p in algo.segment_swa.network.named_buffers():
            print('sma_network buffer', n, p)
            break

        for n, p in algo.network.named_parameters():
            print('network param', n, p[0,0,0])
            break
        for n, p in algo.segment_swa.named_parameters():
            print('sma_network param', n, p[0,0,0])
            break

    # debug_info()
    # flops, params = profile(algo.featurizer, inputs=[torch.randn(1, 3, 224, 224), torch.randn(1, 768, 1, 1)])
    # print(f'total param: {params/2**20:.2f}M')
    for i in range(15):
        minibatch = [(torch.randn(4, 3, 224, 224).to(device), torch.randint(0, 7, (4, )).to(device)) for i in range(1)]
        # print('predict', i)
        # print(algo.predict(minibatch[0][0]))
        print('update', i)
        print(algo.update(minibatch, None))
        # if i % 5 == 0:
        #     print('update and evaluate')
        #     algo.update_and_evaluate(i*0.01)
        # debug_info()
